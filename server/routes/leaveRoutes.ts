...
// GET /api/leaves - Scoped leave requests
...
// POST /api/leaves - Submit leave request with validation
router.post('/', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (!employeeId) {
      return res.status(400).json({
        success: false,
        error: 'Current user has no associated employee record.',
      });
    }
...
// PUT /api/leaves/:id/approve - Manager, HR, Admin (Self-approval strictly forbidden)
...
// PUT /api/leaves/:id/reject - Manager, HR, Admin
