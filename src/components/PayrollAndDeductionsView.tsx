...
export const PayrollAndDeductionsView: React.FC<PayrollAndDeductionsViewProps> = ({
  ...
  const [activeSubTab, setActiveSubTab] = useState<'late_tracking' | 'penalties' | 'payroll_sheet'>('late_tracking');
  ...
  const overallTotals = useMemo(() => {
    ...
    return {
      ...
      totalAppliedDeductions: Math.round((lateDeductions + penaltyDeductions) * 100) / 100,
  ...
  const payslipSummary = useMemo(() => {
    ...
    return calculateMonthlyEmployeeLateSummary(viewingPayslipUser, attendanceRecords, deductions, selectedMonth);
  }, [viewingPayslipUser, attendanceRecords, deductions, selectedMonth]);
